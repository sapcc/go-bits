...
func SerializeCompactContextToJSON(c policy.Context) ([]byte, error) {
	...
	if isAdminProject := a["is_admin_project"]; isAdminProject != "" {
		s.IsAdminProject = isAdminProject
	}
...
type serializedContext struct {
	...
	IsAdminProject        string   `json:"ap,omitempty"` // only if token is an admin project token
...
func DeserializeCompactContextFromJSON(buf []byte) (policy.Context, error) {
	...
	auth["is_admin_project"] = s.IsAdminProject
