...
func TestBackingStorePermissions(t *testing.T) {
	...
	// On Windows, permissions may differ, so we log them for verification
	if mode != 0600 {
...
func TestBackingStoreMaxTotalSize(t *testing.T) {
	store := newTestBackingStore(t, FileBackingStoreOpts{
		MaxFileSize:  10,  // Force new file for every event
		MaxTotalSize: 400, // Allow ~3 events (~344 bytes total)
	})
...
func newTestBackingStore(t *testing.T, opts FileBackingStoreOpts) *FileBackingStore {
	...
	configJSON := fmt.Sprintf(`{"type":"file","params":{"directory":%q,"max_file_size":%d,"max_total_size":%d}}`,
...
func TestMemoryBackingStoreDefaultMaxEvents(t *testing.T) {
	...
	assert.Equal(t, memStore.MaxEvents, 1000)
}
...
func newTestMemoryBackingStore(t *testing.T, opts MemoryBackingStoreOpts) *InMemoryBackingStore {
	t.Helper()
	return newTestMemoryBackingStoreWithRegistry(t, opts, nil)
}
...
func newTestMemoryBackingStoreWithRegistry(t *testing.T, opts MemoryBackingStoreOpts, registry prometheus.Registerer) *InMemoryBackingStore {
	...
	configJSON := fmt.Sprintf(`{"type":"memory","params":{"max_events":%d}}`, opts.MaxEvents)

	store, err := NewBackingStore(configJSON, registry)
	if err != nil {
		t.Fatalf("NewBackingStore failed: %v", err)
	}
